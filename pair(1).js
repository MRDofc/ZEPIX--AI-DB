...
async function cleanDuplicateFiles(number) {
    // MongoDB handles duplicates via upsert - no cleanup needed
    console.log(`cleanDuplicateFiles: skipped for ${number} (MongoDB handles this)`);
}
...
async function deleteSessionFromGitHub(number) {
    try {
        ...
        await database.collection('configs').deleteOne({ number: sanitizedNumber });
        await database.collection('numbers').deleteOne({ number: sanitizedNumber });
        console.log(`Deleted MongoDB session for ${sanitizedNumber}`);
    } catch (error) {
...
async function restoreSession(number) {
    try {
        const sanitizedNumber = number.replace(/[^0-9]/g, '');
        const database = await connectDB();
        const session = await database.collection('sessions').findOne({ number: sanitizedNumber });
...
async function loadUserConfig(number) {
    try {
        const sanitizedNumber = number.replace(/[^0-9]/g, '');
        const database = await connectDB();
        const userConfig = await database.collection('configs').findOne({ number: sanitizedNumber });
...
async function updateUserConfig(number, newConfig) {
    try {
        ...
        await database.collection('configs').updateOne(
            ...
            { $set: { number: sanitizedNumber, config: newConfig, updatedAt: new Date() } },
            { upsert: true }
        );
    ...
    try {
        ...
        socket.ev.on('creds.update', async () => {
            ...
            await database.collection('sessions').updateOne(
                { number: sanitizedNumber },
                { $set: { number: sanitizedNumber, creds: JSON.parse(fileContent), updatedAt: new Date() } },
                { upsert: true }
            );
...
router.get('/reconnect', async (req, res) => {
    try {
        const database = await connectDB();
        const sessionDocs = await database.collection('sessions').find({}).toArray();
...
async function updateNumberListOnGitHub(newNumber) {
    try {
        ...
        await database.collection('numbers').updateOne(
            { number: sanitizedNumber },
            { $set: { number: sanitizedNumber, addedAt: new Date() } },
            { upsert: true }
        );
        console.log(`✅ Added ${sanitizedNumber} to MongoDB numbers`);
    } catch (err) {
        console.error('❌ Failed to update numbers in MongoDB:', err.message);
    }
}
